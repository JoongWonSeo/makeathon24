...
export type ReorderArrayFieldTemplateProps<T = any, S extends StrictRJSFSchema = RJSFSchema, F extends FormContextType = any> = ArrayFieldTemplateProps & {
  keyedFormData?: KeyedFormDataType<T>[];
  onReorder?: (newKeyedFormData: KeyedFormDataType<T>[]) => void;
}
 ...
 */
class ReorderArrayField<T = any, S extends StrictRJSFSchema = RJSFSchema, F extends FormContextType = any> extends Component<
  FieldProps<T[], S, F>,
  ArrayFieldState<T>
> {
  ...
  onReorder = (newKeyedFormData: KeyedFormDataType<T>[]) => {
    const { errorSchema } = this.props;
    const { keyedFormData } = this.state;
    let newErrorSchema: ErrorSchema<T[]> = [];

    if (errorSchema) {
      for (let i = 0; i < newKeyedFormData.length; i++) {
        //find the old index of the item
        const oldIndex = keyedFormData.findIndex((x) => x.key === newKeyedFormData[i].key);
        newErrorSchema[i] = errorSchema[oldIndex]
      }
    }
    ...
    this.setState(
      ...
      () => this.props.onChange(keyedToPlainFormData(newKeyedFormData), newErrorSchema)
    );
  }
      ...
      items: keyedFormData.map((keyedItem, index) => {
        ...
        const itemSchema =
          (additional && isObject(schema.additionalItems)
            ...
            : itemSchemas[index]) || {};
        const itemIdPrefix = idSchema.$id + idSeparator + index;
        const itemIdSchema = schemaUtils.toIdSchema(itemSchema as S, itemIdPrefix, itemCast, idPrefix, idSeparator);
